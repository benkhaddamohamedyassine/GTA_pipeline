"""Stage 11 -- small, bounded residual-hole filling only. NOT diffusion, NOT
a general-purpose Telea inpaint. Stage 4's AI outpainting is what supplies
most surrounding coverage; this stage just cleans up leftover point-cloud-
rasterization gaps. Its output feeds Stage 12 (post-warp super-resolution).
"""
...
def run(
    ...
    filled_path = paths.stage_output_path("11_filled_warp", relative_path)
    filled_mask_path = paths.stage_output_path("11_filled_mask", relative_path, ext_override=".png")
    large_unsupported_path = paths.sidecar_path("11_filled_mask", relative_path, ".large_unsupported_mask.npy")

    outputs = [filled_path, filled_mask_path, large_unsupported_path]
    ...
    try:
        ...
        atomic_write_image((~final_large_unsupported).astype(np.uint8) * 255, filled_mask_path)
